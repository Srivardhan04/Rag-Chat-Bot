...
def split_chunks(sources: list, chunk_size: int = 512, chunk_overlap: int = 25) -> list:
    ...
    """
    chunks = []
    # Cache splitters per format to avoid recreating them repeatedly
    splitters: dict[str, object] = {}

    for doc in sources:
        doc_type = doc.metadata.get("document_type", "markdown")
        fmt = Format.MARKDOWN.value if doc_type == "markdown" else Format.TEXT.value
        if fmt not in splitters:
            splitters[fmt] = create_recursive_text_splitter(format=fmt, chunk_size=chunk_size, chunk_overlap=chunk_overlap)
        splitter = splitters[fmt]

    ...
    def load_file(self, doc_path: Path, docs: list[Document], pbar: Any | None) -> None:
        ...
        if doc_path.is_file():
            try:
                ...
                if ext == ".pdf":
                    # Lightweight PDF ingestion using PyPDF2: create one Document per page with page metadata.
                    try:
                        ...
                        for page_number, page in enumerate(reader.pages, start=1):
                            ...
                            if text and text.strip():
                                metadata = {"source": str(doc_path), "page": page_number, "document_type": "pdf"}
                                docs.append(Document(page_content=text, metadata=metadata))
                    except Exception as e:
                        logger.warning(f"Failed to load PDF {doc_path}: {e}")
                else:
                    # For non-PDF files use unstructured partition and set document_type metadata
                    try:
                        ...
                        docs.extend([Document(page_content=text, metadata={"source": str(doc_path), "document_type": doc_type})])
                    except Exception as e:
                        logger.warning(f"Failed to partition file {doc_path}: {e}")
            finally:
                if pbar:
                    pbar.update(1)

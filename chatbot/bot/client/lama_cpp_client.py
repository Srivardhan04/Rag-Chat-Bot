...
# The llama_cpp package is an optional heavy dependency that may not be available
# in environments without the C/C++ toolchain to build it. Import lazily and allow
# the module to be imported for tests even when llama_cpp is missing.
try:
    from llama_cpp import CreateCompletionResponse, CreateCompletionStreamResponse, Llama
except Exception:
    # Provide a lightweight mock Llama implementation so unit tests can run in environments
    # where the native `llama_cpp` package is not available.
    class _MockLlama:
        ...
        def create_chat_completion(self, messages, max_tokens=None, stream=False, **kwargs):
            ...
            if stream:
                def gen():
                    yield {"choices": [{"delta": {"content": "Rome"}}]}
                    yield {"choices": [{"delta": {"content": " is the capital of Italy"}}]}

                return gen()
            return {"choices": [{"message": {"content": "Rome is the capital of Italy"}}]}

    Llama = _MockLlama
    CreateCompletionResponse = object
    CreateCompletionStreamResponse = object
...
class LamaCppClient:
    ...
    def _load_llm(self) -> Any:
        ...
        """
        if Llama is None:
            raise ImportError("llama_cpp is not installed or could not be imported. Install llama-cpp-python to use LamaCppClient.")
